...
var ASM_CONSTS = {
 ...
 4254195: function() {
  infocontext = new (window.AudioContext || window.webkitAudioContext)();
  if (!infocontext) {
   return 0;
  }
  inforate = infocontext.sampleRate;
  infocontext.close();
  delete infocontext;
  return inforate;
 },
 4254396: function() {
  mInputRegistered = false;
 },
 4254426: function() {
  ...
  FMOD_JS_MixFunction = Module.cwrap("FMOD_JS_MixFunction", "void", [ "number" ]);
  return context.sampleRate;
 },
 4254632: function($0, $1) {
  _as_script_node = context.createScriptProcessor($1, 0, $0);
 },
 4254696: function() {
  window.addEventListener("touchend", OutputWebAudio_resumeAudio, false);
  window.addEventListener("click", OutputWebAudio_resumeAudio, false);
  mInputRegistered = true;
 },
 4254866: function() {
  if (mInputRegistered) {
   mInputRegistered = false;
   window.removeEventListener("click", OutputWebAudio_resumeAudio, false);
   window.removeEventListener("touchend", OutputWebAudio_resumeAudio, false);
  }
 },
 4255069: function() {
  _as_script_node.connect(context.destination);
  _as_script_node.onaudioprocess = function(audioProcessingEvent) {
   _as_output_buffer = audioProcessingEvent.outputBuffer;
   FMOD_JS_MixFunction(_as_output_buffer.getChannelData(0).length);
  };
 },
 4255307: function() {
  _as_script_node.disconnect(context.destination);
 },
 4255360: function($0, $1, $2, $3) {
 ...
 4255881: function() {
  mWorkletNode = null;
  mModulePolling = false;
  mModuleLoading = false;
  mStartInterval = null;
  mStopInterval = null;
  mSuspendInterval = null;
  mResumeInterval = null;
  mWorkletNodeConnected = false;
  mInputRegistered = false;
  mContext = new (window.AudioContext || window.webkitAudioContext)();
  if (!mContext) {
   return 0;
  }
  mContext.destination.channelCount = mContext.destination.maxChannelCount;
  return mContext.destination.maxChannelCount;
 },
 4256322: function($0) {
  ...
  if (!mContext.audioWorklet.addModule) {
   return -2;
  }
  FMOD_JS_MixerSlowpathFunction = Module.cwrap("FMOD_JS_MixerSlowpathFunction", "void", []);
  FMOD_JS_MixerFastpathFunction = Module.cwrap("FMOD_JS_MixerFastpathFunction", "void", [ "number" ]);
  return mContext.sampleRate;
 },
 4256715: function($0, $1) {
  mSpeakerChannelCount = $0;
  const bufferLength = $1;
  mUrl = null;
  mOutputData = null;
  mSharedArrayBuffers = false;
  if (self.SharedArrayBuffer) {
   if (self.crossOriginIsolated) {
    mSharedArrayBuffers = true;
   }
  }
  if (!mSharedArrayBuffers) {
   ...
   mUrl = URL.createObjectURL(slowCodePath);
   mOutputData = new Float32Array(mSpeakerChannelCount * bufferLength);
  } else {
   ...
   mUrl = URL.createObjectURL(fastCodePath);
   mOutputData = new Float32Array(new SharedArrayBuffer(Float32Array.BYTES_PER_ELEMENT * mSpeakerChannelCount * bufferLength * 2));
  }
  mModulePolling = true;
  return mSharedArrayBuffers;
 },
 4260242: function() {
  window.addEventListener("touchend", OutputAudioWorklet_resumeAudio, false);
  window.addEventListener("click", OutputAudioWorklet_resumeAudio, false);
  mInputRegistered = true;
 },
 4260420: function() {
  if (mStartInterval) {
   clearInterval(mStartInterval);
  }
  if (mStopInterval) {
   clearInterval(mStopInterval);
  }
  if (mSuspendInterval) {
   clearInterval(mSuspendInterval);
  }
  if (mResumeInterval) {
   clearInterval(mResumeInterval);
  }
  if (mWorkletNode) {
   if (mWorkletNodeConnected) {
    mWorkletNode.disconnect();
   }
   delete mWorkletNode;
  }
  if (mContext) {
  ...
  if (mInputRegistered) {
   mInputRegistered = false;
   window.removeEventListener("click", OutputAudioWorklet_resumeAudio, false);
   window.removeEventListener("touchend", OutputAudioWorklet_resumeAudio, false);
  }
 },
 4261009: function() {
  mStartInterval = waitForAudioWorklet(function() {
   return mWorkletNode === null && mModulePolling;
  }, function() {
   if (mWorkletNode) {
    mWorkletNode.connect(mContext.destination);
    mWorkletNodeConnected = true;
   }
  });
 },
 4261227: function() {
  mStopInterval = waitForAudioWorklet(function() {
   return mWorkletNode === null && mModulePolling || !mWorkletNodeConnected;
  }, function() {
   if (mWorkletNode) {
    mWorkletNode.disconnect();
    mWorkletNodeConnected = false;
 ...
 4261454: function($0, $1, $2) {
  ...
  for (var channel = 0; channel < speakerModeChannels; channel++) {
   ...
   for (var sample = 0; sample < bufferLength; sample++) {
    mOutputData[sample + offset] = data[sample * speakerModeChannels + channel];
   }
  }
 },
 4261874: function($0, $1, $2, $3) {
  const buffer = $0;
  const bufferLength = $1;
  const speakerModeChannels = $2;
  const frameFlag = $3;
...
var dynCall_iififiiii = Module["dynCall_iififiiii"] = createExportWrapper("dynCall_iififiiii");
